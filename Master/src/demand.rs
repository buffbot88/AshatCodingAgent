...
//!
//! Spawn-on-demand pool of `llama-server` children used by both the 230M
//! Orchestrator (18079 baseline + 18078/18077 extras) and the 1.2B Coding
...
use crate::types::{DemandAcquireError, DemandSpawnError, Pool};
...
impl DemandPool {
    ...
    /// is busy or the spawn health-check fails.    /// Try to immediately spawn a fresh instance. Returns `None` if every port
...
async fn wait_for_health(port: u16, host: &str) -> Result<(), DemandSpawnError> {
    ...
    Err(DemandSpawnError::HealthGaveUp(format!(
        "{url} never reported healthy in 15 s"
    )))
}
