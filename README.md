...
- **retired model text Execution lane (intent router)** — always-on baseline on port `18079` carrying the router GGUF in `models/`, supervised by `demand.rs`. Spawn-on-demand extras on `18078` / `18077` if load climbs and the baseline saturates.
...
| GET | `/api/public_status` | none | execution lane + coding-agent snapshots (3 lanes) |
...
| GET | `/v1/models` | none | execution lane + coding-agent labels |
...
- GGUF files are auto-discovered from `models/*.gguf`. Hints in the config pin filenames when ambiguous; otherwise the execution lane = smallest GGUF, the inference model = first `1.2B`-`Instruct` GGUF.
- The text execution lane binds port `18079`, `18078`, `18077`; the coding-agent binds `18080`, `18081`, `18082`. Adjust in your local `server-config.json` (a copy of the tracked `server-config.example.json`) if a port is occupied on your host.

## Host setup

A prebuilt **llama-server** binary ships at `bin/llama-server`; alternatively install it from <https://github.com/ggerganov/llama.cpp> and place it on `PATH` or point at it via `ASHAT_LLAMA_BIN`. Make sure the GGUF files are inside `models/` (the retired model text-router and 1.2B Instruct GGUFs).
...
- `crates/omega-core` — demand pools, queues, execution lane, weighted router, supervision, tool loop, external skills integration
