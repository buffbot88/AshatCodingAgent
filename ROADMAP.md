...
| --- | ----- | --------- | ----- |
| **retired model Text Router (Execution lane)** | Intent classification (`chat/code/status/unknown`); correlates request ↔ 1.2B; routes responses back to caller. May scale under load. | Baseline always-on. Extras spawn-on-demand, kill-after-task. | `18079` (baseline); grows into `18078` → `18077`. (Total peak: 3×retired model.) |
...
| **BrainStem proxy** | Omega itself, listens on `:8080`. Routes through the execution lane pool, then the coding-agent pool. | Always-on. | `0.0.0.0:8080`. |
...
| Phase | Scope | Status |
| ----- | ----- | ------ |
| **1** | Omega server baseline — Execution lane pool, Coding Agent pool, row-chain wiring (beta/delta disabled). | **done** |
...
| **8** | MySQL external skills integration — the router execution lane seeds coding-agent workspaces from the Ashat skills base (`workspace.rs`, `workspaces/agent-{port}/`). The `skill` tool exists (Phase 6): enable `skills_db.enabled: true` and build with `--features omega-core/skills-db` once connection details land. | deferred (seam reserved) |
...
| MySQL external skills integration → workspace seeding | `workspace.rs` in `crates/omega-common`; the Phase 6 `skill` tool is implemented but gated — set `skills_db.enabled: true` with real connection details and build with `--features omega-core/skills-db`. |
